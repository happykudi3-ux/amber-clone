...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { rows: insertedRows } = await sql`
      ...
      RETURNING id
    `;
    const employeeMessageId = insertedRows[0].id;
    ...
    try {
      const scored = await scoreSentiment(message);
      await sql`
        UPDATE conversations
        SET sentiment = ${scored.sentiment || null},
            concern_score = ${scored.concern_score || null},
            tags = ${JSON.stringify(scored.tags || [])}
        WHERE id = ${employeeMessageId}
      `;
    } catch (err) {
